...
function receiveData(data){
	...
	var docs = eval(data.docs);
	
	docs.forEach(function(row){
		var highlight = highlighting[row.brdno];
		
		var brdwriter= highlight && highlight.brdwriter ? highlight.brdwriter : row.brdwriter;
		var brdtitle = highlight && highlight.brdtitle ? highlight.brdtitle : row.brdtitle;
		var brdmemo  = highlight && highlight.brdmemo ? highlight.brdmemo[0] : row.brdmemo;
		
		var childNode = $('<div  class="panel panel-default"/>'); 
        var html = '<div class="panel-body">' + 
				   '		<div class="col-lg-12 title"><a href="boardRead?brdno=' + row.brdno + '" target="_blank">' + brdtitle + '</a></div>' + 
				   '		<div class="col-lg-12 row">...' + brdmemo.substring(0, 200) + '...</div>' +  
				   '		<div class="col-lg-12 row">' + 
				   '        	<div class="pull-left">등록자: ' + brdwriter + '</div>' +  
				   '         	<div class="pull-left field1">등록일: ' + row.brddate+ ' ' + row.brdtime+ '</div>' + 
				   '		</div>' + 
			       '</div>';
		childNode.html(html)
	...
	///////////////////////////////// 
	if (searchType==="") {
		jQuery.each(data.facet, function(key, val) {
			$("#cnt"+key).html(val);
		});
	}
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header"><i class="fa fa-gear fa-fw"></i> Solr 테스트</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
